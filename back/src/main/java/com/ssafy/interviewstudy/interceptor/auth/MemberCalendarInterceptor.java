...
@Component
public class MemberCalendarInterceptor implements HandlerInterceptor {
    ...
    @Autowired
    public MemberCalendarInterceptor(CalendarService calendarService, MemberService memberService) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        int calendarId = pathVariables.get(1);
        ...
        //내 일정인지 아닌지 체크
        if( !calendarService.checkOwnCalendar(memberId,calendarId) ){
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED,"본인 일정이 아닙니다");
            return false;
        }
        return true;
