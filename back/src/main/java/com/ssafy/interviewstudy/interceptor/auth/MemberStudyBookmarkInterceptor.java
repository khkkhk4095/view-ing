...
public class MemberStudyBookmarkInterceptor implements HandlerInterceptor {
    ...
    @Autowired
    public MemberStudyBookmarkInterceptor(MemberStudyBookmarkService memberStudyBookmarkRepository, MemberService memberService) {
        this.memberStudyBookmarkService = memberStudyBookmarkRepository;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        Boolean isStudyBookmarkByMemberId =
                memberStudyBookmarkService.checkStudyBookmarkByMemberId(memberId,studyId);

        if(!isStudyBookmarkByMemberId){
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,"잘못된 JWT 정보");
            return false;
        }
        return true;
