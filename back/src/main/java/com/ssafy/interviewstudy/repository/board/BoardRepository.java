...
public interface BoardRepository extends JpaRepository<Board, Integer> {
    ...
    // 검색조건 : 제목 제목+내용 작성자
    @Query("select b from Board b JOIN b.author ba where b.title like %:keyword% and b.boardType = :boardType")
    Page<Board> findByTitleContaining(@Param("keyword") String keyword, @Param("boardType") BoardType boardType, Pageable pageable);

    // 제목+내용
    @Query("select b from Board b JOIN b.author ba where b.title like %:keyword% or b.content like %:keyword% and b.boardType = :boardType")
    Page<Board> findByTitleOrContent(@Param("keyword") String keyword, BoardType boardType, Pageable pageable);

    // 작성자
    @Query("SELECT b FROM Board b JOIN b.author ba where ba.nickname like %:keyword% and b.boardType = :boardType")
    Page<Board> findWithAuthor(@Param("keyword") String keyword, BoardType boardType, Pageable pageable);

    // 내가 쓴 모든 글 조회
    @Query("SELECT b FROM Board b JOIN b.author ba where ba.id = :memberId")
    Page<Board> findAllByMemberId(Integer memberId, Pageable pageable);

    // 내가 쓴 글 조회(게시판 별로)
    @Query("select b from Board b JOIN b.author ba where ba.id = :memberId and b.boardType = :boardType")
    Page<Board> findByMemberIdAndBoardType(Integer memberId, BoardType boardType, Pageable pageable);

    // 글 리스트 조회
    @Query("select b from Board b JOIN b.author ba where b.boardType = :boardType")
